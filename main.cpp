...
const string defaultTests[] = { "file1.txt", "file2.txt", "file3.txt", "file4.txt", "file5.txt" };
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        for (auto file : defaultTests)
            test_files.push_back(file);
    }

    for (string test_file : test_files) {
        string out_file = "out_" + test_file;
        ...
        ifs.open(test_file); // open the file to read from
        ofs.open(out_file); // open the file to write to

        pathptr = new Pathfinder();

        if (!ifs.is_open()) { // if the file did not open, there was no such file
            cout << "File " << test_file << " could not open, please check your lab setup" << std::endl;
        }
        else {
            cout << "Reading " << test_file << "..." << std::endl;
        }

        cout << "Beginning " << out_file << " write" << std::endl;
        ...
        cout << "File write complete" << std::endl << std::endl;
        if (pathptr != NULL) {
            delete pathptr;
            pathptr = NULL;
        }
	...
	cout << "end" << endl; // indicate that the program has successfully executed all instructions
	return 0;
}
...
void parse_instruction(string temp, ofstream &ofs, Pathfinder* aptr) {
	...
	if (command == "importMaze") { // command to import a new maze from a file
		...
		ofs << temp << "\n" << aptr->toString() << endl << is_valid_path(pathVec, aptr->toString()) << std::endl; //<< ":\n" << ss.str() << std::endl;
	}
